...
def net2grid(*,  # enforce the use of keyword arguments
    ...
    if num_x_blocks > 0 and num_y_blocks > 0:
        ...
        if verbose:
            print(f"  : Grid calculation: x_dist={x_dist_km:.1f}{unit}, y_dist={y_dist_km:.1f}{unit}")
            print(f"  : Cell size: {cell_width}{unit} x {cell_height}{unit}")
            print(f"  : Grid dimensions: {num_x_blocks} x {num_y_blocks} = {num_x_blocks * num_y_blocks} zones")
    ...
    # select zones that contain nodes
    if node_dict:
        # create array of node points for efficient spatial filtering
        node_points = np.array([(node["x_coord"], node["y_coord"]) for node in node_dict.values()])

        # filter zones that contain at least one node
        overlapping_zone_ids = []
        for grid_id, grid_info in grid_dict.items():
            zone_geom = grid_info["geometry"]
            x_min, x_max = grid_info["x_min"], grid_info["x_max"]
            y_min, y_max = grid_info["y_min"], grid_info["y_max"]

            # fast bbox check first
            in_bbox = ((node_points[:, 0] >= x_min) & (node_points[:, 0] <= x_max) &
                      (node_points[:, 1] >= y_min) & (node_points[:, 1] <= y_max))
        ...
        if is_geometry:
            ...
            input_polygons = shapely.MultiPoint([(zone["x_coord"], zone["y_coord"]) for zone in zone_dict.values()]).convex_hull
        ...
        overlapping_zone_ids = list(grid_dict.keys())
    ...
    for idx, grid_id in enumerate(overlapping_zone_ids):
        ...
        grid_val["id"] = idx + 1
        grid_dict_overlapping[f"{idx + 1}"] = grid_val
